...
import { useAuth } from "@/context/AuthContext"; // Adjust the path to your useAuth hook
...
export default function Navbar() {
  const { user } = useAuth(); // Assuming useAuth provides user and logout
            ...
            {user ? (
              <>
                <button
                  type="button"
                  // onClick={logout}
                  className="text-white text-sm gap-2 font-medium flex items-center"
                >
                  Logout
                </button>
                {/* Profile dropdown */}
